...
// CTA final: el padre escribe su reflexión y se envía al WhatsApp
// del proyecto (wa.me abre el chat con el texto listo para enviar).
function CtaWhatsapp() {
  const [texto, setTexto] = useState('')

  const enviar = () => {
    const mensaje = `¡Hola! Hice el quiz de estilos de crianza 🦅\n\n${texto.trim()}`
    window.open(
  ...
  return (
    <div className="curso-cta">
      ...
      <p>
        ¿Con qué estilo te identificas más? ¿Y qué quisieras mejorar para ser
        más un papá o mamá Águila? Escríbelo aquí y envíanoslo — nos encanta
        leerte.
      </p>
      <textarea
        className="curso-cta__texto"
        value={texto}
        onChange={(e) => setTexto(e.target.value)}
        rows={4}
        placeholder="Ejemplo: Me identifico más con el Pato 🦆. Quisiera mejorar en sostener los límites sin ceder cuando mi hijo insiste…"
        aria-label="Cuéntanos con qué estilo te identificas y qué quisieras mejorar"
      />
      <button
        onClick={enviar}
        disabled={!texto.trim()}
        className="cta-link cta-link--whatsapp curso-cta__enviar"
      >
